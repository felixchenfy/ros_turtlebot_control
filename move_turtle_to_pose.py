...
def test_complex_move(args, turtle):
    ...
    turtle.move_to_relatvie_pose(-0.5, 0.5, -np.pi/2) # P4: go back to origin
    # turtle.move_to_pose(-0.5, -0.5, -np.pi/2) # P3: go back to P1
...
def test_move_to_pose(turtle):
    turtle.set_pose(x=0.5, y=0.5, theta=np.pi/2)
    # turtle.move_to_relative_pose(0.5, 0.5, np.pi/2) # P2: go half a circle
    # turtle.move_to_relative_pose(-0.5, -0.5, -np.pi) # P2: go half a circle
    # turtle.move_to_relative_pose(-0.5, -0.5, 0) # P2: go half a circle
    turtle.move_to_relative_pose(-0.5, -0.5, None) # P2: go half a circle
    # turtle.move_to_relative_pose(0.5, 0.5, None) # P2: go half a circle
